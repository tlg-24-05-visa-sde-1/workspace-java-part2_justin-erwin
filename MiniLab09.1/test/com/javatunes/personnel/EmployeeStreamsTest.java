...
public class EmployeeStreamsTest {
    ...
    public void testSalaryLessThanSortNaturalOrder() {
        List<Employee> employees = allEmployees.stream()
                ...
                .collect(toList());     //we can use this instead of .collect(Collectors.toList()) b/c of our static import
    ...
    public void findAll_nameLengthAtMost5_sortBySalary() {
        List<Employee> result = allEmployees.stream()
                ...
                .sorted(comparing(Employee::getSalary))   //b/c of the static import of Comparator, we can do comparing instead of Comparator.comparing
                .collect(toList());
        dump(result);
    }
    ...
    public void threeHighestPaid_sortByName() {
        Collection<Employee> highestPaid = allEmployees.stream()
    ...
    public void findAllNames_hired2010Later() {
        List<String> names = allEmployees.stream()
                .filter(employee -> employee.getHireDate().getYear() >= 2010)
                .map(employee -> employee.getName())     //Note: this is a Stream<String> now, employees went in, and Strings came out
    ...
    public void giveRaise_salaryLessThan50000_hiredAfter2000_lowestPaidFirst() {
        List<Employee> raiseList =  allEmployees.stream()
    ...
    boolean allAfter2000 = allEmployees.stream()
            .allMatch(emp -> emp.getHireDate().getYear()>=2000);

        System.out.println(allAfter2000);
    }
